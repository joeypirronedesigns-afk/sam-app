module.exports = async function handler(req, res) {
  ...
  if (mode === 'calendar') {
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, system: calPrompt, messages: [{ role: 'user', content: moment }] }) });
  ...
  if (mode === 'ideas') {
    ...
    try {
      const r = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, system: ideasPrompt, messages: [{ role: 'user', content: moment }] }) });
      if (!r.ok) throw new Error('API error ' + r.status);
      const d = await r.json();
      return res.status(200).json(JSON.parse(d.content?.[0]?.text?.replace(/```json|```/g, '').trim()));
    } catch (err) { return res.status(500).json({ error: err.message || 'Something went wrong.' }); }
  ...
  const textPostInstruction = 'Write a complete text post ready to copy and paste. CRITICAL: Do NOT use [BRACKETS], (pacing notes), beat labels, or any video/audio directions. Write in clean paragraphs only. Hook on the first line, story in short punchy paragraphs, ends with a question or CTA. Pure readable text.';
  ...
  const contentTypeScriptInstructions = {
    ...
    'LinkedIn text post': 'Write the complete LinkedIn post as clean text only. No [BRACKETS]. Strong opening, short paragraphs, ends with a question. 3 hashtags at the end.',
    'Instagram caption': 'Write the complete Instagram caption as clean text only. No [BRACKETS]. Hook first, body with line breaks, CTA, 5 hashtags.',
    'Podcast intro': 'Write a complete 60-90 second spoken intro. Hooks listener, sets theme, teases content.',
    'Email newsletter': 'Write complete email as clean text. SUBJECT LINE first line. PREVIEW TEXT second line. Then body — no [BRACKETS]. Greeting, 3-4 paragraphs, CTA.',
    'Blog post': 'Write as clean text. HEADLINE first. META DESCRIPTION second. Intro paragraph, section headers in ALL CAPS, 2-3 sentence summaries, conclusion with CTA.',
  ...
  try {
    const anthropicRes = await fetch('https://api.anthropic.com/v1/messages', { method: 'POST', headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 2000, system: systemPrompt, messages: [{ role: 'user', content: moment }] }) });
    if (!anthropicRes.ok) { const e = await anthropicRes.json().catch(() => ({})); throw new Error(e.error?.message || 'API error ' + anthropicRes.status); }
    const anthropicData = await anthropicRes.json();
    return res.status(200).json(JSON.parse(anthropicData.content?.[0]?.text?.replace(/```json|```/g, '').trim()));
  } catch (err) { return res.status(500).json({ error: err.message || 'Something went wrong.' }); }
};
