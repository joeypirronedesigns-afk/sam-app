module.exports = async function handler(req, res) {
  ...
  const callAnthropic = async (system, userContent, maxTokens) => {
    ...
    try {
      ...
      clearTimeout(timeout);
      if (e.name === 'AbortError') throw new Error('Request timed out — try a shorter moment description');
      throw e;
    }
  };
  ...
  try {
    ...
    if (mode === 'upload') {
      ...
      const uploadSystem = [
        ...
        'IF photo: {"type":"photo","what_sam_sees":"one sentence on what is in the image","thumbnail_strategy":"specific crop direction and composition advice","thumbnail_headline":"bold 3-6 word text overlay for the thumbnail — ALL CAPS, punchy","thumbnail_subtext":"optional 2-4 word supporting line below headline","thumbnail_emotion":"one word: the face/emotion to show — e.g. SHOCKED, PROUD, DETERMINED","thumbnail_color":"one dominant accent color as hex code that would pop e.g. #FF4500","hook_ideas":["hook 1","hook 2","hook 3"],"best_platform":"single best platform","platform_reason":"one sentence why","content_angle":"the scroll-stopping angle","caption_starter":"first line of a caption"}',
